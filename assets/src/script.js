...
function updateTable(data) {
	...
	data.forEach(function(entry) {
		...
		let linebadge = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}"><div class="linebadge ${entry.line.product} ${entry.line.name.replace(/\s/g, '')}${entry.line.operator.id} ${entry.line.operator.id} ${entry.line.productName}">`;
		...
		if ((siteType === 'S' || siteType === 'L') && timediff <= 60) {
			if (entry.when !== null) {
				if (timediff <= 0) {
					countdownCell.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}">jetzt</a>`;
					...
					if (delayDifference > 5) {
						...
						countdownCell.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}"><span style="color: #ec0016;">` + timediff + `<span class="additional">&nbsp;min.</span></span></a>`;
					} else {
						countdownCell.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}">` + timediff + '<span class="additional">&nbsp;min.</span></a>';
					}
					...
					setInterval(function() {
						timediff = Math.round((departureTime - new Date()) / (1000 * 60));
						if (timediff <= 60) {
							countdownCell.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}">${timediff} min.</a>`;
						} else {
							countdownCell.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}">${formatTime(entry.when)}</a>`;
						}
					}, 60000);
				}
			}
			...
			if (entry.when !== null) {
				if (delayDifference > 0) {
					countdownCell.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}"><nobr class='mobilebreak'><s class='disabled'>${formatTime(entry.plannedWhen)}</s> ${formatTime(entry.when)}</nobr></a>`;
				} else {
					countdownCell.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}"><span class="timetable">${formatTime(entry.when)}</span></a>`;
				}
			} else {
				countdownCell.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}"><span class="timetable">${formatTime(entry.plannedWhen)}</span></a>`;
			}
		}
		...
		if (isCancelled) {
			if (entry.plannedPlatform === null) {
				if (entry.platform === null) {
					row.insertCell(3).innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}"></a>`;
				} else {
					row.insertCell(3).innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}"><s>${entry.platform}</s></a>`;
				}
			} else {
				row.insertCell(3).innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}"><nobr class='mobilebreak'><s class='disabled'>${entry.plannedPlatform}</s></nobr></a>`;
			}
		} else {
			if (entry.platform == null) {
				row.insertCell(3).innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}">-</a>`;
			} else if (entry.platform == entry.plannedPlatform) {
				row.insertCell(3).innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}${encodeURIComponent(entry.tripId)}">${entry.plannedPlatform}</a>`;
			} else if (entry.plannedPlatform === null) {
				row.insertCell(3).innerHTML = `<a class="red" href="trip.html?tripId=${encodeURIComponent(entry.tripId)}">${entry.platform}</a>`;
			} else {
				row.insertCell(3).innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}"><nobr class='mobilebreak'><s class='disabled'>${entry.plannedPlatform}</s><span class="red"> ${entry.platform}</span></nobr></a>`;
			}
		}
		...
		if (siteType !== 'A') {
			wideCell2.innerHTML = `<a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}"><span class="scrolling-wrapper"><span class="scrolling-text"> ${entry.direction} </span></span> </a>`;
		} else {
			wideCell2.innerHTML = `<span class="prefix">Von&nbsp;</span><a href="trip.html?tripId=${encodeURIComponent(entry.tripId)}">${entry.provenance}</a>`;
		}
